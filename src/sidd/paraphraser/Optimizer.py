...
def minWindowEstimator(indices, transObjectList):
    minIndex=10000
    for transObject in transObjectList:
        set1=set(indices)
        set2=transObject.indices
        ...
        if not commonElem:
            ...
            min_s1=min(set1)
            min_s2=min(set2)
            if min(min_s1, min_s2) > minIndex:
                minIndex=min(min_s1, min_s2)
    return minIndex  

def formLMSequence(wordObjectList,transObject, indices,transObjectList, l_window_size=2):
    #sequence=''
    r_window_size=2
    min_index=minWindowEstimator(indices, transObjectList)
    if min_index == 10000:
        min_index=min(indices)
    diff=min_index-l_window_size
    while(diff<0):
            l_window_size=l_window_size - 1
            diff=min_index-l_window_size
...
def solveILP(transObjectList, wordObjectList, lm, wordFrequency, favorCompression=False, favorSimpleWords=True ):
    ...
    # Create variables
        ...
        for transObject in transObjectList:
            ...
            #coeff2=coeff2*len(transObject.sourceSequence.split())
            ...
            if tokenDiff<1.0 and tokenDiff!=0 and favorCompression is True:
                coeff2 = (1/tokenDiff)*coeff2 + 1
